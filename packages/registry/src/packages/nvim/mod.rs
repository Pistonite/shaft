...
pub fn install(ctx: &Context) -> cu::Result<()> {
    ...
    let file_name = nvim_file_name()?;
    let file_stem = cu::check!(Path::new(file_name).file_stem(),"failed to get nvim file stem")?;
...
pub fn configure(ctx: &Context) -> cu::Result<()> {
    ...
    ctx.add_item(Item::shim_bin(bin_name!("nvim"), ShimCommand::target(install_bin_str)))?;
    ctx.add_item(Item::shim_bin(bin_name!("vim"), ShimCommand::target(install_bin_str)))?;
    ctx.add_item(Item::shim_bin(bin_name!("vi"), ShimCommand::target(install_bin_str)))?;
    ...
    // invoke config
    cu::which("python")?.command().args(["setup.py", "apply"])
        .current_dir(&config_dir).stdoe(cu::lv::P).stdin_null().wait_nz()?;
...
pub fn clean(_: &Context) -> cu::Result<()> {
    cu::which("python")?.command().args(["setup.py", "clean"])
        .current_dir(nvim_config_dir()?).stdoe(cu::lv::P).stdin_null().wait_nz()?;
    Ok(())
}
...
fn nvim_config_dir() -> cu::Result<PathBuf> {
    if cfg!(windows) {
        ...
        let mut p = cu::check!(std::env::home_dir(), "failed to get user home dir to figure out nvim config dir")?;
