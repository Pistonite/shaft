...
pub fn configure(ctx: &Context) -> cu::Result<()> {
    ...
    let cfg_delta = config.get("delta").and_then(|x|x.as_bool()).unwrap_or(true);
    if cfg_editor {
        command_output!("git", ["config", "--global", "core.editor", "viopen"]);
    } else {
        command_output!("git", ["config", "unset", "--global", "core.editor"]);
    }
    command_output!("git", ["config", "--global", "core.autocrlf", &cfg_autocrlf.to_string()]);
    if cfg_delta {
        command_output!("git", ["config", "--global", "core.pager", "delta"]);
        command_output!("git", ["config", "--global", "interactive.diffFilter", "delta --color-only"]);
        ...
        command_output!("git", ["config", "--global", "merge.conflictStyle", "zdiff3"]);
    } else {
        command_output!("git", ["config", "unset", "--global", "core.pager"]);
        command_output!("git", ["config", "unset", "--global", "interactive.diffFilter"]);
        ...
        command_output!("git", ["config", "unset", "--global", "merge.conflictStyle"]);
    }
    VERSION.update()?;
    Ok(())
}
