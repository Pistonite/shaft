...
pub fn init_arch_with_uname(default: CpuArch) -> cu::Result<()> {
        match get_arch_with_uname() {
            Ok(Some(x)) => CURRENT_ARCH.set(x),
            Ok(None) => {
...
fn get_arch_with_uname() -> cu::Result<Option<CpuArch>> {
    let (child, stdout) = cu::which("uname")?.command().arg("-m").stdout(cu::pio::string())
    .stdin_null().stderr_null().spawn()?;
    child.wait_nz()?;
