...
use winreg::enums::{HKEY_LOCAL_MACHINE, HKEY_CURRENT_USER, KEY_WRITE};
...
fn get_from_key_path(name: &str, key: HKEY, subpath: &str) -> cu::Result<String> {
    let reg_key = cu::check!(RegKey::predef(key).open_subkey(subpath), "open_subkey failed")?;
...
fn set_from_key_path(name: &str, key: HKEY, subpath: &str, value: &str) -> cu::Result<()> {
    let reg_key = cu::check!(RegKey::predef(key) .open_subkey_with_flags( subpath, KEY_WRITE),"failed to open_subkey with write flag")?;
    cu::check!(reg_key.set_value(name, &value), "failed to set reg key value")
}
