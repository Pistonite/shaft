...
struct Cli {
    ...
    /// Specify the line ending to use [default: lf, crlf if core.autocrlf is true]
    ///
    ...
    /// Don't respect ignore files (`.ignore` or `.gitignore`)
    ///
    /// `.lfmtignore` is always respected reguardless of this setting
    #[clap(short = 'N', long)]
    no_ignore: bool,

    /// Don't respect the core.autocrlf git config [only effective on Windows and when -n is not
    /// specified]
    ///
    /// Without this flag (the default), when running without -n on Windows and `git config
    /// core.autocrlf` returns true:
    ///   - Check is skipped
    ///   - When fixing, CRLF will be used instead of LF
    #[clap(long, conflicts_with = "no_ignore")]
    no_autocrlf: bool,
...
async fn main(args: Cli) -> cu::Result<()> {
    ...
    let end = end.unwrap_or(LineEnd::Lf);
