...
pub fn check_init_binary() -> cu::Result<()> {
    ...
    if home_binary != current_exe {
        ...
        if let Err(e) = check_binary_location(&home_binary) {
            cu::error!("{e:?}");
            cu::warn!("binary location check failed - please ensure your PATH is configured correctly; restarting the terminal/shell is recommended");
...
pub fn copy_new_binary(new_binary: &Path) -> cu::Result<()> {
    ...
    let home_binary = op::home::shaft_binary();
    let home_binary_old = op::home::shaft_binary_old();
    cu::check!(cu::fs::remove(&home_binary_old), "failed to remove old old binary")?;
    if home_binary.exists() {
        cu::check!(cu::fs::copy(&home_binary, &home_binary_old), "failed to copy old binary")?;
        cu::check!(cu::fs::remove(&home_binary), "failed to remove old binary")?;
    }
    cu::check!(cu::fs::copy(new_binary, &home_binary), "failed to copy new binary to home")?;
...
fn check_binary_location(home_binary: &Path) -> cu::Result<()> {
    ...
    if home_binary != &binary_in_path {
        cu::bail!("binary in PATH is not the same as home: '{}' (home is '{}')", binary_in_path.display(), home_binary.display());
    }
    Ok(())
}
...
pub fn upgrade_binary(path: Option<&Path>) -> cu::Result<()> {
    ...
    let new_binary = match path {
        Some(path) => {
            let cargo = cu::check!(cu::which("cargo"), "cannot find `cargo` - cargo is required to upgrade from local path.")?;
            cu::info!("installing to cargo default location...");
            {
                let (child, _progress, _progress2) = cargo.command()
                    .current_dir(path)
    ...
    cu::check!(copy_new_binary(&new_binary), "failed to copy new binary to home")?;
